...
public class ClockPanel extends JPanel {
	...
	private ClockWindow mParent;

	public ClockPanel(ClockWindow parent, SettingDataHolder holder) {
		mParent = parent;
		mDataHolder = holder;
	}
	...
	/**
	 * ウィンドウのサイズ、形状、透過性を変更します。
	 */
	private void changeWindowProperty() {
		// サイズの変更
		mParent.setSize(mDataHolder.getGuiWidth(), mDataHolder.getGuiHeight());
		// 背景の透過性と形状を変更
		float opacity = mDataHolder.getOpacity();
		Shape shape;
		if (mDataHolder.roundRectangle()) {
			shape = mDataHolder.getRoundRectangleShape();
		} else {
			shape = mDataHolder.getRectangleShape();
		}
		mParent.updateShape(opacity, shape);
	}
	...
	private void drawText(Graphics2D g2, String text, Font font, int x, int y) {
		...
		if (mDataHolder.isRainbow()) {
			...
			LinearGradientPaint gradient = new LinearGradientPaint(new Point2D.Float(0,
					(int) (timeY * 1.1)), new Point2D.Float((int) (strHeight * 0.4),
					(int) (timeY * 1.1) + (int) (strHeight * 0.4)), RainbowUtil.DIST,
	...
	private void drawAnalogClock(Graphics2D g2) {
		...
		double minX = centerX + (Math.sin(Math.toRadians(min * 6 + (sec * 6 / 60)))) * radius
				* 0.75;
		double minY = centerY - (Math.cos(Math.toRadians(min * 6 + (sec * 6 / 60)))) * radius
				* 0.75;
		double hourX = centerX + (Math.sin(Math.toRadians(hour * 30 + (min * 6 / 12)))) * radius
				* 0.6;
		double hourY = centerY - (Math.cos(Math.toRadians(hour * 30 + (min * 6 / 12)))) * radius
				* 0.6;
